...
- Hotfix identidade/outbox `0.8.41`: validar `go test ./internal/domain/usecase -run 'Test(CollectAndBuffer|CachedIdentityHeaderProvider|FlushOutbox)' -count=1 -v`; coletores devem renovar `X-Agent-Identity`, replay local da outbox deve trocar credencial expirada antes do envio, e envelopes `meta.via=hub` devem preservar a identidade original.
