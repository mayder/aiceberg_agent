...
- Validacao real parcial em `VMAIPROD2`/Oracle Linux em 2026-06-19: agente 0.8.8 ativo com canal direct, ping online, flush ACK e snapshots para agente 4; `scripts/smoke.sh` oficial passou no host com binarios pre-compilados, backend fake local, `health.status=ok`, ingestao bootstrap/health/metrics/logs e servico real ainda `active` com `NRestarts=0`; API indisponivel passou com binario temporario, porta local fechada, `bootstrap degraded` sem fatal, health OK, canal em fallback, retry de bootstrap e recuperacao de bootstrap/ping/config/ingest quando a API volta. Servidores Inspect tiveram smoke real parcial: `old-plesk-92.204.168.1`/agente esperado `19` manteve `aiceberg-agent.service` ativo com `NRestarts=0` durante smoke temporario, evidencia SHA-256 `91992345bba689fc4e1e5bc20f769bb58873e398750f7504838c92482b5f39f8`; `187.45.180.181` passou smoke POSIX temporario, evidencia SHA-256 `b325aeea690f72c853d595239c85451c1925e34b72334fe28e6955d82ee5f20b`, mas sem unit instalada e com CPU curta acima do limite inicial; `old-plesk-92.42.106.121` bloqueou acesso nao interativo. Ainda pendem rollback real, update remoto assinado e demais ambientes/cenarios do gate.
