...
- Validacao real parcial em `VMAIPROD2`/Oracle Linux em 2026-06-19: agente 0.8.8 ativo com canal direct, ping online, flush ACK e snapshots para agente 4; `scripts/smoke.sh` oficial passou no host com binarios pre-compilados, backend fake local, `health.status=ok`, ingestao bootstrap/health/metrics/logs e servico real ainda `active` com `NRestarts=0`; API indisponivel passou com binario temporario, porta local fechada, `bootstrap degraded` sem fatal, health OK, canal em fallback, retry de bootstrap e recuperacao de bootstrap/ping/config/ingest quando a API volta. Rollback controlado em systemd no mesmo host foi validado e versionado em `docs/evidence/pkg69/linux-rhel-20260619T025113Z`, com restore do binario atual, `NRestarts=0`, flush ACK e CPU `3.4%`. Servidores Inspect tiveram validacao real parcial: agentes `1`, `18` e `19` atualizados para binario 0.8.8 com `NRestarts=0`, canal direct/flush ACK e seen recente no AIceberg; no Debian 11 do agente `19`, rollback controlado foi validado e versionado em `docs/evidence/pkg69/linux-debian-20260619T030202Z`, com restore do binario atual, `NRestarts=0`, flush ACK e CPU `3.0%`; `187.45.180.181` tem agente `70` instalado em modo usuario/crontab, health local OK em `18081`, seen recente no AIceberg e coleta read-only adicional em `/tmp/aiceberg_pkg69_linux70_readonly_20260619T020840Z` com artefato SHA-256 `28e3bfd6cae62bd67750172971ab19933d5d48dc99d04f01f519377ccb6e8110`; `METRICS_INTERVAL=60` foi instalado no agente `70` com binario SHA-256 `54ee6fe293d30222845c9780b63d0bbb80331c0477ef73399f1f8d468c8ac397`, backup local e CPU estabilizada em `1.7%`, mas systemd/root segue pendente por falta de `sudo -n`; Windows Server 2022 `10.100.35.3`/agente `71` passou smoke Windows, foi instalado como servico `AIcebergAgent` `Running`, health local OK em `8081` e recebeu `METRICS_INTERVAL=60` com binario SHA-256 `950d73174467ec3bbca4fac9ba255c9a9bc458d68fbee165fbedd370ae3b4c4e`, CPU estabilizada em `1.33%`, canal direct conectado e `relay_connects_to_aiceberg=false`; AIceberg tinha `seen=2026-06-19 01:49:34`. Rollback controlado do Windows Server foi validado e versionado em `docs/evidence/pkg69/windows-server-20260619T023855Z`. Docker Runtime passou no gate com bundle `docs/evidence/pkg69/docker-runtime-20260619T031843Z`, daemon Docker real local, container controlado, logs JSON com cursor, `containers_seen=1`, `container_logs_seen=7`, CPU `0.0%`, RSS `9261056` e cleanup validado. Permissao eBPF/PCAP restrita passou no gate com bundle `docs/evidence/pkg69/permission-ebpf-20260619T032753Z`, `tcpdump` sem permissao em Linux real, warning claro, ingest ativo no agente `70` e CPU `1.4%`. Alto volume e overhead passou no gate com bundle `docs/evidence/pkg69/high-volume-overhead-20260619T033436Z`, DogStatsD e OTLP metrics/logs/traces com `accepted_count=1300`, `dropped_count=740`, CPU `0.0%` e RSS `14567688`. Proxy/TLS passou no gate com bundle `docs/evidence/pkg69/proxy-tls-20260619T033945Z`, proxy autenticado, TLS invalido rejeitado, TLS valido aceito, `requests_ok=2`, `requests_failed_expected=1` e `retry_count=0`. Disco cheio passou no gate com bundle `docs/evidence/pkg69/disk-full-20260619T034504Z`, volume APFS temporario de 16MB, erro real `no space left on device`, outbox preservada e recuperacao validada. Relay/Hub/Direct passou no gate com bundle `docs/evidence/pkg69/relay-hub-direct-hosts-20260619T035749Z`, containers separados, Relay visto pelo backend via IP do Hub, `relay_direct_api_attempts=0` e Agentless via Hub validado. Update remoto e rollback passou no gate com bundle `docs/evidence/pkg69/remote-update-rollback-20260619T040722Z`, artefato remoto assinado Ed25519, SHA256 validado, `version_confirmed`, `update_report_status=apply_failed` e rollback do apply script em container Debian root isolado. Kubernetes RBAC minimo passou no gate com bundle `docs/evidence/pkg69/kubernetes-rbac-20260619T041959Z`, cluster kind efemero, DaemonSet 1/1, Helm install/upgrade/rollback, events/logs coletados e `auth can-i` negando `secrets`, `pods/exec` e `delete pods`. Clock/NTP controlado passou no gate com bundle `docs/evidence/pkg69/clock-skew-20260619T042607Z`, offset `180000 ms`, `time_sync.status=critical`, retorno `ok` e sem alterar clock do host. O gate ficou em 12/14 evidencias OK. Ainda pendem Windows desktop e reboot durante coleta. Diagnostico atual: WinRM nao esta exposto no Windows informado, apenas RDP, e o host validado e Windows Server; reboot real exige autorizacao explicita e janela em host nao critico.
