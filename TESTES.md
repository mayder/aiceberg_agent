...
- Gate de fechamento: `scripts/pkg69_operational_evidence_gate.sh` gera/valida manifest TSV de evidencias reais e campos criticos por conteudo; `scripts/pkg69_operational_evidence_gate_selftest.sh` roda em `./check.sh`, incluindo rejeicao de `relay_direct_api_attempts > 0`, CPU/RSS acima dos limites iniciais e RBAC Kubernetes com `secrets/exec/delete` permitido.
