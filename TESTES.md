...
- Gate de fechamento: `scripts/pkg69_operational_evidence_gate.sh` gera/valida manifest TSV de evidencias reais com SHA256/tamanho do template e do anexo bruto; `scripts/pkg69_operational_evidence_gate_selftest.sh` roda em `./check.sh`, incluindo rejeicao de `Data UTC` fora de `YYYY-MM-DDTHH:MM:SSZ`, topologia placeholder, evidencia sem anexo bruto local existente ou com anexo vazio, rollback nao validado, `relay_direct_api_attempts > 0`, Relay com campo textual de conexao direta marcado como nao seguro, Agentless sem comprovacao via Hub, CPU/RSS acima dos limites iniciais, status invalido de clock/update, metricas numericas obrigatorias e RBAC Kubernetes com `secrets/exec/delete` permitido. `scripts/pkg69_bundle_evidence_selftest.sh` tambem roda em `./check.sh` e valida o helper `scripts/pkg69_bundle_evidence.sh`, incluindo bloqueio de cenario desconhecido. `scripts/pkg69_collect_host_evidence_selftest.sh` roda em `./check.sh` e valida o coletor read-only `scripts/pkg69_collect_host_evidence.sh`. `scripts/pkg69_run_evidence_gate_from_bundles_selftest.sh` roda em `./check.sh` e valida integridade SHA256/tamanho dos bundles, manifest com uma unica evidencia, `created_at_utc` em `YYYYMMDDTHHMMSSZ`, artefato do manifest coerente com `Evidencia bruta anexada`, cenario desconhecido e mapeamento de bundles reais para o gate oficial sem aceitar fechamento incompleto. `scripts/pkg69_evidence_gap_report_selftest.sh` roda em `./check.sh` e valida o relatorio Markdown de pendencias/invalidos a partir do manifest, incluindo modo bloqueante `PKG69_GAP_REPORT_REQUIRE_COMPLETE=true`.
