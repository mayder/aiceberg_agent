...
func parsePodLogLine(line string, p pod, spec containerSpec, maxBytes int) (map[string]any, string, bool) {
	...
	applyOriginAnnotations(event, p.Metadata.Annotations)
	soclog.EnrichMap(event)
	return event, timestamp, true
}

func applyOriginAnnotations(event map[string]any, annotations map[string]string) {
	for annotation, field := range map[string]string{
		...
		if value := strings.TrimSpace(annotations[annotation]); value != "" {
			event[field] = value
