...
- `187.45.180.181` Linux, agente `70`: cadastro criado no AIceberg para o cliente `1`, modo direct, mas instalacao systemd bloqueada porque o usuario `deploy` nao tem `sudo -n`; continua aguardando senha sudo/root ou outro procedimento autorizado;
...
| --- | --- | --- |
| Windows Server | `smoke.ps1`, EventLog, servico Windows, update/rollback | parcial real: Windows Server 2022 id `71` com smoke, servico `AIcebergAgent` instalado e health OK; update/rollback real pendente |
| Windows desktop | `smoke.ps1`, EventLog, instalador, proxy | pendente real |
| Ubuntu/Debian | `smoke.sh`, systemd, logs, outbox, update | parcial real: Debian 11 id `1`, id `18` e id `19` atualizados com systemd ativo e envio ao AIceberg; Ubuntu 24.04 id `70` cadastrado, mas instalacao bloqueada por falta de sudo; update/rollback e CPU dentro do limite ainda pendentes |
