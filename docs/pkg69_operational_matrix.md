...
- Reboot real segue pendente de ambiente controlado.
- Diagnostico em 2026-06-19: Windows desktop real `GOVI` (`192.168.15.24`, Windows 10 Home Single Language) foi acessado via AnyDesk, WinRM foi habilitado e validado em `5985`, o bundle `docs/evidence/pkg69/windows-desktop-20260619T125957Z` passou no gate e o reboot real durante coleta foi executado com agente `72`/ativo `153`. O Linux novo `187.45.180.181:1158` segue sem `sudo -n`, mas o gate Debian/RHEL ja cobre systemd/root em hosts reais.
...
- Clock/NTP controlado validado em 2026-06-19 com container isolado, `time.google.com` apontado por DNS local para servidor NTP controlado no proprio container, offset `180000 ms` reportando `time_sync.status=critical`, retorno com offset zero reportando `status=ok`, backend local capturando ingest de metrics e sem alterar o relogio do host; bundle versionado em `docs/evidence/pkg69/clock-skew-20260619T042607Z`;
- Windows desktop validado em 2026-06-19 no host `GOVI` (`192.168.15.24`, Windows 10 Home Single Language) via AnyDesk + WinRM. O smoke Windows usou binario `0.8.9` pre-compilado e backend local, o instalador oficial criou `AIcebergAgent`, EventLog System/Application foi coletado, o servico ficou `Running`, health local retornou `status=ok`, `proc_cpu_percent=2.057598728498627`, `proc_rss_bytes=34537472`, e o servico de teste foi parado/desabilitado ao final para nao manter token dummy em execucao. Bundle versionado em `docs/evidence/pkg69/windows-desktop-20260619T125957Z`; `scripts/pkg69_evidence_gap_report.sh docs/evidence/pkg69` com `PKG69_WINDOWS_DESKTOP_EVIDENCE` reporta `13/14` evidencias OK;
- Windows desktop real com token do cliente validado em 2026-06-19 no mesmo host `GOVI`: agente `72`, ativo `153`, versao `0.8.10`, servico `AIcebergAgent` `Running`, canal direct conectado, `OSLOG_ENABLED=true`, `OSLOG_MIN_SEVERITY=error`, canais `Application,System`, cursor persistente em `C:\ProgramData\AIceberg\data\oslogs.cursor`. A coleta Windows foi corrigida para ler `wevtutil` em XML, normalizar nivel localizado `Erro` para `error` e aplicar severidade minima no XPath (`Level<=2`) antes do limite do lote. Evento controlado `6907`/RecordID `8474` foi coletado e persistido no backend em `log_bruto.id=17163824`, `log_nivel_id=3`, `fonte_log.id=21`, `ativo_id=153`, `repeticoes=1`, com health local `status=ok`, `version=0.8.10`, CPU `1.4046416530304082%` e RSS `36896768`. Evidencia em `docs/evidence/pkg69/windows-desktop-real-client1-20260619T164239Z/evidence.md`; isso valida ingestao real de EventLog do cliente, mas nao substitui o gate pendente de reboot real;
- Reboot real durante coleta validado em 2026-06-19 no Windows desktop `GOVI`: antes do reboot, health reportou `endpoint_backlog` com `/v1/ingest/bootstrap=1` e `/v1/ingest/metrics=1`; a maquina reiniciou com boot UTC `2026-06-19T16:48:09Z`; o servico `AIcebergAgent` voltou automaticamente `Running`; o outbox persistente `C:\ProgramData\AIceberg\data\outbox.db` permaneceu integro; API real foi restaurada; health final ficou `status=ok`, `version=0.8.10`, CPU `1.3975441293285926%`, canal direct conectado; backend registrou apos o boot `metrics_after_reboot.total=6`, `bootstrap_after_reboot.total=1` e `agente.ultimo_seen_em=2026-06-19 16:54:40`; bundle versionado em `docs/evidence/pkg69/reboot-during-collection-20260619T165440Z`;
- update remoto real via web validado em 2026-06-19 no canario `VMAIPROD2`/agente `4`: artefatos oficiais `0.8.9` publicados em `cliente/web/downloads/agent/0.8.9/`, tela `agente/view?id=4` agendou `v0.8.9 • Linux AMD64`, auto-update policy habilitada apenas no canario, primeira tentativa estritamente gerada pela UI falhou de forma controlada por timeout ao baixar `https://cliente.aiceberg.com.br` a partir do proprio host; a pendencia local foi corrigida com split-horizon `cliente.aiceberg.com.br -> 127.0.0.1` e inclusao da intermediate `GlobalSign GCC R6 AlphaSSL CA 2025` na trust store do VMAIPROD2, com `openssl verify` OK e `curl -I https://cliente.aiceberg.com.br/downloads/agent/0.8.9/aiceberg-agent-linux-amd64.tar.gz` retornando `HTTP/2 200`; update forçado posterior usando a URL publica HTTPS original concluiu `download`, `apply`, restart systemd e `version_confirmed`; health final `status=ok`, `version=0.8.9`, canal direct conectado, binario instalado SHA-256 `a25d50df4aec1cbb798d5afcac963b7848a84fcb7c8dcf8a6e7e124f42bfa78d`, igual ao binario extraido do pacote Linux amd64; evidencia em `docs/evidence/pkg69/remote-web-update-20260619T113327Z/evidence.md`; isso valida update remoto real no canario com URL publica HTTPS funcional no proprio host, mas nao substitui update assinado Ed25519 nem fecha os gates globais de PKG-69;
- rollout remoto completo do cliente `1` validado em 2026-06-19: agentes ativos `1`, `4`, `18`, `19`, `70` e `71` atualizados para `0.8.9` via backend web, com `last_update_status=version_confirmed` e `versao_agente=0.8.9` em todos. O Windows `71` validou update remoto real com pacote `aiceberg-agent-windows-amd64.zip` apos reenfileiramento usando launcher em `D:\AIcebergAgentInstall`. O Linux usuario `70` aplicou o binario por update remoto, exigiu restart manual por PID path incorreto e teve o comando remoto corrigido para `/home/deploy/aiceberg-agent/data/agent.pid`. Evidencia em `docs/evidence/pkg69/remote-web-update-client1-20260619T121826Z/evidence.md`. Isso valida rollout web multiagente do cliente, mas nao fecha os gates globais de PKG-69;
...
| Windows desktop | `smoke.ps1`, EventLog, instalador, proxy | OK no gate: bundle `docs/evidence/pkg69/windows-desktop-20260619T125957Z`, Windows 10 Home real, WinRM, smoke, instalador, servico Windows, EventLog e overhead validados; proxy fica coberto no cenario dedicado `proxy-tls` |
...
| Reboot durante coleta | servico retorna e outbox preserva dados | OK no gate: bundle `docs/evidence/pkg69/reboot-during-collection-20260619T165440Z`, reboot real em Windows desktop, servico voltou, outbox preservado e backend confirmou ingest apos boot |
