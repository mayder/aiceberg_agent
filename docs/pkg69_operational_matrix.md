...
| --- | --- | --- |
| Windows Server | `smoke.ps1`, EventLog, servico Windows, update/rollback | parcial real: Windows Server 2022 id `71` com smoke, servico `AIcebergAgent` instalado, health OK e `METRICS_INTERVAL=60` reduzindo CPU para ~1.33%; update/rollback real pendente |
| Windows desktop | `smoke.ps1`, EventLog, instalador, proxy | pendente real |
| Ubuntu/Debian | `smoke.sh`, systemd, logs, outbox, update | parcial real: Debian 11 id `1`, id `18` e id `19` atualizados com systemd ativo e envio ao AIceberg; Ubuntu 24.04 id `70` instalado em modo usuario/crontab e envio ao AIceberg; `METRICS_INTERVAL=60` reduziu CPU para ~1.7%; systemd/root e update/rollback ainda pendentes |
