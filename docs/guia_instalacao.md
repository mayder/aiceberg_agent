...
- Overhead de sysmetrics: `METRICS_INTERVAL` controla o intervalo da coleta principal de metricas do host. O default segue `10` segundos para compatibilidade; em hosts com CPU acima do limite operacional inicial, aumente para `30` ou `60` e valide `/health.proc_cpu_percent`, `/metrics` e ingestao antes de fechar o gate.
- Coleta de logs (SOC):
