...
Para rodar o gate a partir de uma pasta com bundles ja coletados, use `scripts/pkg69_run_evidence_gate_from_bundles.sh <pasta-bundles>`. O script valida que cada `MANIFEST.tsv` tem uma unica linha de evidencia, `created_at_utc` em formato compacto `YYYYMMDDTHHMMSSZ`, confere SHA256/tamanho do `evidence.md` e do anexo bruto, mapeia cada cenario para a variavel `PKG69_*_EVIDENCE` correta e chama o gate oficial; com `PKG69_REQUIRE_REAL_EVIDENCE=true`, continua falhando enquanto qualquer cenario obrigatorio estiver ausente.
